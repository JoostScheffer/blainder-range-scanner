...
def castRay(
    ...
    if closestHit is not None:
        ...
        # eq. (19)
        receivedSignalLevel = (
            sourceLevel - 2 * transmissionLoss - noiseLevel + directivityIndex + processingGain
        )  # + targetStrength
                ...
                if simulateWaterProfile:
                    ...
                    for layerIndex in range(firstValueBelowSensor, len(depthList) + 1):
                        ...
                        if debugLines:
                            generic.addLine(
                                internalOrigin,
                                internalOrigin + direction * min(remainingDistance, newRange),
                            )
                ...
                if closestHit is not None:
                    ...
                    if "partID" in closestHit.target:
                        ...
                        partIDIndex = closestHit.target.material_slots[
                            materialMappings[closestHit.target][closestHit.faceIndex]
                        ].name
